...
export function SlideLayer({
  ...
  useEffect(() => {
    const loop = () => {
      ...
      slide.layers.forEach((l, j) => {
        ...
        // Per-layer parallax during transition:
        // faster-parallax layers (high py) travel further → real depth feel
        const py = l.py || 10;
        const scrollY = isEx
          ? td * -tp * py * 3.2
          : td * (1 - tp) * py * 3.2;
        ...
        el.style.transform = base
          ? `${base} translate(${mouseTx}px,${mouseTy + scrollY}px)`
          : `translate(${mouseTx}px,${mouseTy + scrollY}px)`;
      });
  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex, transform: `translateY(${translateY}%) scale(${scale})`, filter: blur > 0.15 ? `blur(${blur}px)` : 'none', opacity, willChange: 'transform, filter, opacity', transformOrigin: 'center center' }}>
      ...
      <div
        ...
        style={{ position: 'absolute', inset: 0, zIndex: 10, display: 'flex', alignItems: 'center', padding: '0 8%', gap: '4%', opacity: textVisible ? 1 : 0, transition: 'opacity 0.32s ease' }}
      >
        <div style={isUfoSlide ? { width: '100%', textAlign: 'center' } : { flex: 1 }}>
          ...
          >
            {slide.headline.map((line, k) => (
              <span
                key={k}
                style={{ display: 'block', animation: animateText ? `fadeUp 0.72s cubic-bezier(0.16,1,0.3,1) ${0.04 + k * 0.07}s both` : 'none' }}
              >
                {line}
              </span>
            ))}
        ...
        <div
          ...
          style={{ width: 290, textAlign: 'center', marginTop: isUfoSlide ? 14 : 0, animation: animateText ? 'fadeUp 0.72s cubic-bezier(0.16,1,0.3,1) 0.28s both' : 'none' }}
        >
